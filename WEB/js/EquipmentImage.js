function EquipmentChangeImage(actionSelected) {
    ...
    $("#ChangeImageDialog").removeClass("hide").dialog({
        "resizable": false,
        "modal": true,
    ...
    if (window.File && window.FileReader && window.FileList && window.Blob) {
        ...
        switch (ftype) {
            ...
            case "image/pjpeg":
                break;
            default:
                alertUI("<strong>" + ftype + "</strong>&nbsp;" + Dictionary.Common_Warning_MimeType);
                return false;
        }

        //Allowed file size is less than 50KB (25428)
        if (fsize > 5000000) {
            alertUI("<strong>" + fsize + "</strong>&nbsp;" + Dictionary.Common_Warning_ImageTooBig);
            return false;
        }
...
function EquipmentDeleteImage() {
    $("#DeleteImageDialog").removeClass("hide").dialog({
        "resizable": false,
        "modal": true,
